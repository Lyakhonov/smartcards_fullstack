...
class TestFlashcardWorkflow:
    ...
    async def test_complete_flashcard_workflow(
        ...
        update_response = authenticated_client.put(
            f"/flashcards/{card_id}", json={"question": "2+2=?", "answer": "4"}
        )
        assert update_response.status_code == 200

        # 5. Удаляем карточку
        delete_response = authenticated_client.delete(f"/flashcards/{card_id}")
...
class TestDataIsolation:
    ...
    async def test_users_cannot_access_each_other_data(
        ...
        group = Group(
            id=generate_uuid(), filename="Secret Group", user_id=other_user.id
        )
        test_db.add(group)
        await test_db.commit()
...
class TestErrorHandling:
    ...
    def test_concurrent_modifications(self, authenticated_client, test_user):
        ...
        response = authenticated_client.delete("/groups/nonexistent")
...
class TestBoundaryConditions:
    ...
    @pytest.mark.asyncio
    async def test_large_number_of_flashcards(
        self, authenticated_client, test_user, test_db
    ):
    ...
    def test_very_long_text_in_flashcard(
        self, authenticated_client, test_user, test_db
    ):
        ...
        async def run_test():
            ...
            response = authenticated_client.post(
                ...
                json={"question": long_text, "answer": long_text},
            )
