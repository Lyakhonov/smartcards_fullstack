...
class TestGroupRetrieval:
    ...
    async def test_get_groups_isolation(
        ...
        assert all(g["filename"].startswith("my_group") for g in data)
...
class TestGroupDeletionAccess:
    ...
    async def test_delete_group_requires_manager(
        ...
        group = Group(
            id=generate_uuid(), filename="to_delete.pdf", user_id=test_user.id
        )
        test_db.add(group)
        await test_db.commit()
    ...
    async def test_delete_group_not_found(self, authenticated_client):
        ...
        response = authenticated_client.delete("/groups/nonexistent_id")
